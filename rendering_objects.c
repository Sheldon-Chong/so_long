...
/*   Updated: 2023/12/14 17:05:36 by shechong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_fov(t_enemy *enemy, t_display *display, t_img *char_array)
{
	...
	draw_line(char_array, iso_map(pos),
		iso_map(move_in_dir(pos, enemy->current_angle -20, 15)), 0x30eeff);
	img_on_img(display->img, char_array, (t_xy){iso_map((t_xy){enemy->pos.x * 21, enemy->pos.y * 21}).x
		+ grid.offset_x - 14,
...
int	render_sentry(t_display *display, t_enemy *enemy)
{
	...
	img_on_img(display->img, enemy->animator->frames[enemy->animator->current_frame], (t_xy){iso_map(enemy->current_pos).x + grid.offset_x
